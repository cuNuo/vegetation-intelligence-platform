...
浏览器访问 `http://127.0.0.1:5174`。端口、host 和代理目标已固化在 `frontend/vite.config.js` 与 `frontend/package.json`，因此无需在命令行重复传参。前端支持按钮选择 GeoTIFF、拖拽导入、批量队列和批量处理，不再要求用户手动输入后端绝对路径。波段映射优先读取影像波长和描述；缺失元数据时可按 `GF01`、`LAD08/LAD09`、`SHB02` 原始上传文件名前缀恢复本项目实际预处理波段。其他影像继续按常见顺序兜底，并可在逻辑波段弹窗中手动修正。地图始终保留当前天地图底图，导入后立即定位并先显示 PNG 预览，原始 TIF 瓦片就绪后自动接管。
